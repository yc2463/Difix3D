...
for scene_id in ids:
    ...
    destination_base_path = f"/share/phoenix/nfs04/S7/yc2463/Difix3D_fork/dataset/DL3DV-Benchmark/{scene_id}"
    ...
    # Load the train_test_split json file
    train_test_split_file_path = sorted(glob.glob(f"/share/phoenix/nfs04/S7/yc2463/Difix3D_fork/train_test_split/DL3DV-Benchmark/{scene_id}/train_test_split*.json"))[0]
    with open(train_test_split_file_path, 'r') as fin:
        train_test_split = json.load(fin)
    train_ids = train_test_split['train_ids']
    test_ids = train_test_split['test_ids']
    print(f"Loaded train-test-split from {train_test_split_file_path}")
